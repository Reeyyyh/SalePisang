...
@section('content')
    <div class="p-6 max-w-4xl mx-auto">
        ...
        <form action="{{ route('seller.products.update', $product->id) }}" method="POST" enctype="multipart/form-data"
            ...
            {{-- Image Upload --}}
            <div>
                <label for="image" class="block text-gray-700 font-medium mb-2">Replace Product Image</label>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full border border-gray-300 rounded px-4 py-2 focus:ring focus:ring-blue-200 @error('description') @enderror">{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
